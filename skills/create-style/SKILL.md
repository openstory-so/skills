...
Return `name`, `id`, and a one-line look summary. To use it on a
sequence, pass `style: "<name>"` (the `create-sequence` skill). Follow
`_links.create-sequence` on the style document only if they asked for a
video next.
